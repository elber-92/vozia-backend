...
def criar_gradiente(destino):
    subprocess.run([FFMPEG, "-y", "-f", "lavfi",
                    ...
                    "-frames:v", "1", str(destino)], capture_output=True)
...
def baixar_imagem(cena, pasta, idx, modo):
    ...
    if modo in ("qualidade", "automatico"):
        import urllib.parse
        prompt = (cena[:120] + ", cinematic illustration, high quality, "
...
def api_video():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        for i, cena in enumerate(cenas):
            renderizar_cena(pasta / f"cena_{i+1:03d}.jpg", cena, i + 1, len(cenas),
        ...
        subprocess.run([FFMPEG, "-y", "-i", str(concat), "-i", str(audio),
